...
Fixed ignition sequence: Set Guess -> Solve Fixed -> Refine
"""
...
def _build_adiabatic_flame():
    """Build initial adiabatic flame with robust ignition sequence"""
    gas = ct.Solution('gri30.yaml')
    # Robust ignition parameters
    Tin, p, phi, uin = 300.0, ct.one_atm, 1.0, 0.25
    width = 0.06 

    gas.TP = Tin, p
    gas.set_equivalence_ratio(phi, 'CH4', {'O2': 0.21, 'N2': 0.78, 'AR': 0.01})

    # 1. Define initial grid
    grid_pts = 60
    grid = np.linspace(0, width, grid_pts)

    try:
        f = ct.BurnerFlame(gas, grid=grid)
    except TypeError:
        f = ct.BurnerFlame(gas, width=width)
        # If we couldn't pass grid in constructor, set it manually if possible, 
        # or rely on refinement. But usually modern Cantera supports grid=.
    ...
    # 2. CRITICAL: Set initial guess BEFORE solving
    # If we solve on a flat 300K profile, it converges to "no flame".
    T_guess = np.zeros(len(f.grid))
    # Create a ramp from 300K to 2200K to simulate a flame front
    for i, x in enumerate(f.grid):
        if x < 0.01:
            T_guess[i] = 300.0
        elif x < 0.02:
            # Ramp up
            T_guess[i] = 300.0 + (2200.0 - 300.0) * ((x - 0.01) / 0.01)
        else:
            T_guess[i] = 2200.0

    f.T[:] = T_guess

    # 3. Solve on FIXED grid first (refine_grid=False)
    # This forces the solver to resolve the gradients we just planted
    print("  Solving on fixed grid...")
    try:
        f.solve(loglevel=0, refine_grid=False)
    except Exception as e:
        print(f"  Warning: Fixed grid solve struggled: {e}")

    # 4. Now enable refinement to clean up the solution
    # Don't prune too aggressively
    f.set_refine_criteria(ratio=3.0, slope=0.05, curve=0.05, prune=0.0)
    print("  Refining grid...")
    f.solve(loglevel=0, refine_grid=True)

    print(f"  Grid points: {len(f.grid)} | Max T: {np.max(f.T):.1f} K")

    if np.max(f.T) < 1000:
        # Last ditch effort: try the auto-initial guess
        print("  Attempting auto-initial guess...")
        f.set_initial_guess()
        f.solve(loglevel=0, refine_grid=True)

    if np.max(f.T) < 1000:
        raise RuntimeError('Flame did not ignite. Max T stayed at 300K.')
...
class ThreeStageSolver:
    """
    Three-stage porous burner solver
    """
    def __init__(self, z, design_vars, Tg, u, p, X):
        ...
        # Stage boundaries (using 6cm total width from flame build)
        self.L1 = 0.020
        self.L2 = 0.035
        self.L3 = 0.060
    ...
    def _heat_transfer_coeff(self, dp, eps, u_local, T_avg):
        ...
        Re = max(rho * (u_local/eps) * dp / mu, 1e-6)
        Pr = mu * cp / k_g
    ...
    def solve(self, max_iter=12, tol=10.0):
        ...
        for iteration in range(max_iter):
            ...
            err_s = np.max(np.abs(Ts-Ts_new))
            err_g = np.max(np.abs(Tg-Tg_new))

            if err_s < tol and err_g < tol:
                break
